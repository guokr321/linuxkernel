...
struct board_info {
	...
	u16		queue_start_addr; //队列首地址
	u16		queue_ip_summed; //队列ip数
	u16		dbug_cnt;
...
//把根据网卡信息找到dm9000板卡信息，因为板卡信息和网卡信息是连续分配的，所以从末尾就能找到
 ...
 * Search DM9000 board, allocate space and register it
 检查DM9000板卡，分配空间，并且注册
 */
static int
dm9000_probe(struct platform_device *pdev)
{
	struct dm9000_plat_data *pdata = dev_get_platdata(&pdev->dev);//挂到device上的平台驱动信息
	struct board_info *db;	/* Point a board information structure 板级信息 */
	struct net_device *ndev;        //网卡设备
	struct device *dev = &pdev->dev; //设备
	const unsigned char *mac_src; //mac源地址
	int ret = 0; 
	int iosize; 
	int i;
	u32 id_val;
	...
	struct regulator *power;//电源管理的
	bool inv_mac_addr = false;

	power = devm_regulator_get(dev, "vcc");//拿到电源管理的
	if (IS_ERR(power)) {
		//没有电源管理
...
/*
在一个网络驱动程序中，一般都提供了一个platform_driver结构变量。platform_driver结构
包括了网卡驱动的相关操作函数，通过platform_driver_register()函数注册到内核设备驱动
列表。内核会根据驱动程序中设备描述设置网卡的中断和定时器，并且在网络数据包到来的时
候调用网卡对应的处理函数。通常，网卡需要向内核提供下面几个接口函数：
